document.addEventListener('DOMContentLoaded', () => {
    ...
    function startSilenceDetection() {
        ...
        silenceDetectionInterval = setInterval(() => {
            ...
            if (average > SILENCE_THRESHOLD) {
                ...
                if (!isRecording) {
                    console.log(`Wykryto dźwięk: ${average.toFixed(2)}, rozpoczynam nagrywanie`);
                    startNewRecording();
                }
    ...
    function startNewRecording() {
        ...
        console.log(`Używam formatu nagrywania: ${mimeType || 'domyślny format przeglądarki'}`);
        ...
        // Add bitrate option for better quality (if codec supports it)
        if (mimeType && (mimeType.includes('mp3') || mimeType.includes('mpeg') || mimeType.includes('opus'))) {
            options.audioBitsPerSecond = 128000; // 128kbps
        }
        
        console.log("Opcje nagrywania:", options);
        
        // Create new MediaRecorder instance with options
        try {
            mediaRecorder = new MediaRecorder(microphoneStream, options);
        } catch (e) {
            console.error("Błąd podczas tworzenia MediaRecorder:", e);
            // Fallback to default options
            try {
                mediaRecorder = new MediaRecorder(microphoneStream);
            } catch (fallbackError) {
                console.error("Krytyczny błąd MediaRecorder:", fallbackError);
                showMessage("Nie można nagrywać dźwięku", "error");
                return;
            }
        }
        ...
        mediaRecorder.onstart = () => {
            console.log(`Nagrywanie #${currentRecordingId} rozpoczęte z formatem ${mediaRecorder.mimeType}`);
            isRecording = true;
        };
        
        mediaRecorder.ondataavailable = (event) => {
            console.log(`Otrzymano dane audio: ${event.data.size} bajtów`);
